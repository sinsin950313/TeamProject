...
int CCamera::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...
void CCamera::SetButtonMasks(int nRotateModelButtonMask, int nZoomButtonMask, int nRotateCameraButtonMask)
{
...
void	CCamera::SetObjectView(TVector3 vMax, TVector3 vMin)
{
	TMatrix	matView;
...
void CCamera::CreateViewMatrix(TVector3 vEye, TVector3 vAt, TVector3 vUp)
{
...
void CCamera::CreateProjMatrix(float fNear, float fFar, float fFovY, float fAspectRatio)
{
	m_fFovY = fFovY;
...
void CCamera::Update(TVector4 vDirValue)
{
	TMatrix matRotation;
...
void CCamera::MoveLook(float fValue)
{
	m_vPos += m_vLook * fValue;
}
void CCamera::MoveSide(float fValue)
{
...
void CCamera::SetWindow(int nWidth, int nHeight, float fArcballRadius)
{
	m_WorldArcBall.SetWindow(nWidth, nHeight, fArcballRadius);
	m_ViewArcBall.SetWindow(nWidth, nHeight, fArcballRadius);
}

void CCamera::SetModelCenter(TVector3 vModelCenter)
{
