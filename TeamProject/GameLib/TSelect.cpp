...
void T_BOX::CreateOBBBox(float fExtentX, float fExtentY, float fExtentZ,
	TVector3 vCenter,
	TVector3 vDirX,
	TVector3 vDirY,
	TVector3 vDirZ)
{
	...
	*/
	...
	vPos[3] = this->vCenter + vAxis[0] - vAxis[1] - vAxis[2];
	vPos[4] = this->vCenter - vAxis[0] - vAxis[1] + vAxis[2];
	vPos[5] = this->vCenter - vAxis[0] + vAxis[1] + vAxis[2];
	...
	{
		if (vMax.x < vPos[iPoint].x)
		{
			vMax.x = vPos[iPoint].x;
		}
		if (vMax.y < vPos[iPoint].y)
		{
			vMax.y = vPos[iPoint].y;
		}
		if (vMax.z < vPos[iPoint].z)
		{
			vMax.z = vPos[iPoint].z;
		}

		if (vMin.x > vPos[iPoint].x)
		{
			vMin.x = vPos[iPoint].x;
		}
		if (vMin.y > vPos[iPoint].y)
		{
			vMin.y = vPos[iPoint].y;
		}
		if (vMin.z > vPos[iPoint].z)
		{
			vMin.z = vPos[iPoint].z;
		}
	}
...
void T_BOX::CreateAABBBox(TVector3 max, TVector3 min)
{
	vMax = max;
	vMin = min;
	...
	TVector3 halfSize = vMax - vCenter;
	CreateOBBBox(halfSize.x, halfSize.y, halfSize.z, vCenter);
}

void	T_BOX::UpdateBox(TMatrix& world)
{
	TVector3 axis[3];
	D3DXVec3TransformNormal(&axis[0], &TVector3::UnitX, &world);
	D3DXVec3TransformNormal(&axis[1], &TVector3::UnitY, &world);
	D3DXVec3TransformNormal(&axis[2], &TVector3::UnitZ, &world);
	vCenter.x = world._41;
	vCenter.y = world._42;
	vCenter.z = world._43;
	...
	float* fPercentage)
{
	...
	TVector3 e1 = (v0 - vStart);
	float a0 = D3DXVec3Dot(&vNormal, &e1);
	float fT = a0 / D;
