...
bool    SceneInGame::Frame()
{
	...
	{//halfCount
		if (m_Scene == S_INGAME && !m_bIngame1_EnemyHalfCount && m_iMobDeadCount >= m_Enemies.size() / 2)
		{
			...
			sound->VolumeSet(0.3f);
			sound->Play(true);
			g_pWriter->SetText(WriteText(621, 673, m_ScenarioList.find(L"m_bIngame1_EnemyHalfCount")->second, { 1,1,1,1 }, 3.0f)); //ÏßÄÏπòÏ??ÑÏïäÍ≥†Îç§Î≤ºÎìú?îÍµ∞
		...
		if (m_Scene == S_INGAME2 && !m_bIngame2_EnemyHalfCount && m_iMobDeadCount >= m_Enemies.size() / 2)
		{
			...
			sound->VolumeSet(0.3f);
			sound->Play(true);
			g_pWriter->SetText(WriteText(621, 673, m_ScenarioList.find(L"m_bIngame2_EnemyHalfCount")->second, { 1,1,1,1 }, 3.0f)); //?¥ÏïºÍ∏∞Îäî ?ÑÏßÅ ?ùÎÇòÏßÄ ?äÏïò??
			m_pInterText->m_pWorkList.push_back(new InterfaceFadeInOut(3.0f));
		}
	}
